...
class PrefixLayerT5(nn.Module):
    ...
    def instantiate(self, hidden_dim):
        self.past_key = nn.Parameter(torch.randn(self.prefix_token_num, hidden_dim, device=self.device),
                                     requires_grad=True)
        self.past_value = nn.Parameter(torch.randn(self.prefix_token_num, hidden_dim, device=self.device),
                                       requires_grad=True)
    ...
    def pre_forward(self, *args, **kwargs):
        ...
        if 'position_bias' in kwargs and kwargs['position_bias'] is not None:
            if kwargs['position_bias'].shape[
                -1] != seq_len + self.prefix_token_num:  # Then the position_bias should be re-calculated
        ...
        if 'mask' in kwargs and kwargs['mask'] is not None:
            ...
            if past_key_len + seq_len == mask_len + self.prefix_token_num:
                ...
                kwargs['mask'] = torch.cat(
                    [-torch.zeros((*am.shape[:-1], self.prefix_token_num), dtype=am.dtype, device=am.device), am],
                    dim=-1)
        return args, kwargs
    ...
    def pre_forward(self, *args, **kwargs):
        ...
        if 'attention_mask' in kwargs and kwargs['attention_mask'] is not None:
            ...
            kwargs['attention_mask'] = torch.cat(
                [-torch.zeros((*am.shape[:-1], self.prefix_token_num), dtype=am.dtype, device=am.device), am], dim=-1)
        return args, kwargs
...
class PrefixLayerDistilBert(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        pseudo_prefix = torch.zeros(key.shape[0], self.prefix_token_num, key.shape[2], dtype=key.dtype,
                                    device=key.device)
...
class ReparameterizeFunction(nn.Module):
    ...
    def define_reparameterization_network(self) -> None:
        ...
        """
        self.input_tokens = nn.Parameter(torch.arange(self.prefix_token_num).long(),
...
class PrefixModel(DeltaBase):
    ...
    def add_all_delta_to_backbone(self,
        ...
        if self.reparameterize:
            ...
            self.insert_sequential_module(first_modified_module, delta_module=reparams, name="reparams",
                                          strict=False)
        self.mark_as_delta()
        return module
    ...
    def new_module_like(self, module):
        ...
        if isinstance(module, T5Attention) or isinstance(module, T5LayerSelfAttention):
            ...
            prefixlayer = PrefixLayerT5(prefix_token_num=self.prefix_token_num, num_heads=module.n_heads,
                                        device=module_device)
        elif isinstance(module,
                        MultiHeadSelfAttention):  # MultiHeadSelfAttention didn't provide past_key_value in the interface of the forward function.
            module_device = get_device(module)
            prefixlayer = PrefixLayerDistilBert(prefix_token_num=self.prefix_token_num, device=module_device)
            self.insert_sequential_module(getattr(module, "k_lin"), pre_caller=prefixlayer.key_pre_forward,
                                          post_caller=prefixlayer.key_forward)
            self.insert_sequential_module(getattr(module, "v_lin"), pre_caller=prefixlayer.value_pre_forward,
                                          post_caller=prefixlayer.value_forward)
        elif isinstance(module, BertSelfAttention):
            raise NotImplementedError
        elif isinstance(module, RobertaAttention):
            module_device = get_device(module)
            prefixlayer = PrefixLayerRoberta(prefix_token_num=self.prefix_token_num,
                                             num_heads=module.self.num_attention_heads, device=module_device)
        elif isinstance(module, GPT2Attention):
            module_device = get_device(module)
            prefixlayer = PrefixLayerGPT2(prefix_token_num=self.prefix_token_num, num_heads=module.num_heads,
                                          device=module_device)
        elif isinstance(module, BartAttention):
            module_device = get_device(module)
            prefixlayer = PrefixLayerBart(prefix_token_num=self.prefix_token_num, num_heads=module.num_heads,
                                          device=module_device)
        else:
