...
class SoftPromptLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.instantiate(raw_embedding(torch.tensor([0]).to('cuda')).shape[-1])
    ...
    def pre_forward(self, *args, **kwargs):
        ...
        am.data = torch.cat(
            [torch.ones((*am.shape[:-1], inputs_embeds.shape[-2] - am.shape[-1]), dtype=am.dtype, device=am.device),
             am], dim=-1)
    ...
    def instantiate(self, hidden_dim) -> None:
        ...
        if self.token_init:
            soft_embeds.data = torch.clone(self.raw_embedding(torch.tensor([i for i in range(self.num_tokens)]).to('cuda'))) # to.('cuda')
        else:
...
class SoftPromptModel(DeltaBase):
    ...
    def __init__(self,
        ...
        try:
            ...
            raise AttributeError(
                f"'{type(self.backbone_model)}' object has no attribute 'get_input_embeddings', please pass " +
    ...
    def update_module(self):
        ...
        self.insert_sequential_module(
            self.backbone_model.get_encoder() if self.backbone_model.config.is_encoder_decoder else self.backbone_model,
